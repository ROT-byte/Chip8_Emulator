...
static void chip8_exec_extended_eight(struct chip8 *chip8, unsigned short opcode)
{
    ...
    unsigned short final_four_bits = opcode & 0x000f;
...
{
    unsigned char x = (opcode >> 8) & 0x00f;
...
static void chip8_exec_extended(struct chip8 *chip8, unsigned short opcode)
{
    ...
    unsigned short x = (opcode >> 8) & 0x00f;
    unsigned short y = (opcode >> 4) & 0x00f;
    ...
    case 0xE000:
    {
        switch (opcode & 0x00ff)
        {
        // EX9E - SKP Vx, Skip the next instruction if the key with the valuse of Vx is pressed
        case 0x9e:
            if (chip8_keyboad_is_down(&chip8->keyboard, chip8->registers.V[x]))
            {
                chip8->registers.PC += 2;
            }
            break;
            // Exa1 - SKNP Vx - Skip the next instruction if the key with the value of Vx
        case 0xa1:
            if (!chip8_keyboad_is_down(&chip8->keyboard, chip8->registers.V[x]))
            {
                chip8->registers.PC += 2;
...
void chip8_load(struct chip8 *chip8, const char *buf, size_t size)
{
    ...
    chip8->registers.PC = CHIP8_CHARACTER_SET_LOAD_ADDRESS;
}